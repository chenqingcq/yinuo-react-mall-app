...
class Detail extends React.Component {
  ...
  getDetail = id => {
    let _this = this;
    Actions.getShopDeliveryAddressDetail({ id: id }).then(response => {
      if (response.code == 0) {
        _this.setState({
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (id == undefined) {
          Actions.createShopDeliveryAddress(values).then(response => {
            if (response.code == 0) {
              Utils.dialog.success("保存成功", () => {
                history.push(`/shops/address/index`);
                _this.setState({
                  isLoading: false
                });
              });
          ...
          Actions.updateShopDeliveryAddress(values).then(response => {
            if (response.code == 0) {
    ...
    return (
      <div id="wrap">
        <Breadcrumb className="breadcrumb">
          <Breadcrumb.Item>
            <Link to={`/shops/address/index`}>
              <span>发货地址列表</span>
            </Link>
          </Breadcrumb.Item>
          <Breadcrumb.Item>
            {is_not_add ? "发货地址详情" : "新增发货地址"}
          </Breadcrumb.Item>
        </Breadcrumb>
        <div className="content">
          <Form onSubmit={this.handleSubmit}>
            ...
            <FormItem {...formItemLayout} label="收货地址别名" hasFeedback>
            ...
            <FormItem {...formItemLayout} label="设置为默认地址" hasFeedback>
...
const ShopAddressDetailForm = Form.create()(Detail);
export default ShopAddressDetailForm;
