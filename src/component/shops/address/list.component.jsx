...
class List extends React.Component {
  ...
  getList = (params) => {
    ...
    Actions.getShopDeliveryAddress(params).then(response => {
      this.setState({
  ...
  onDelete = (id, index) => {
    ...
    confirm({
      ...
      content: '',
      onOk() {
        Actions.deleteShopDeliveryAddress({
          id: id
        }).then(result => {
          if (result.code == 0) {
  ...
  onSetDefault = (id,index) => {
    let _this = this;
    Actions.updateShopDeliveryAddress({
      id: id,
      isDefault: 0
    }).then(result => {
      if (result.code == 0) {
    ...
    return (
      <div id="wrap">
        ...
        <div className="content">
          ...
          <div className="table">
            {this.state.list.length === 0 ? <div style={{textAlign:"center"}}>暂无发货地址</div> :this.state.list.map((record, index) => {
...
const ShopAddressList = Form.create()(List);
